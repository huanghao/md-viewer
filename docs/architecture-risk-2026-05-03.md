...
**现状**：main.ts 从 3158 行降到 **395 行**（-87%）。拆分出的模块：
...
**建议**：按职责拆分为 `annotation-state.ts`（纯状态）、`annotation-ui.ts`（Popover/Composer DOM）、`annotation-pdf.ts`（PDF 桥接逻辑）。优先级低于其他高风险项，但值得规划。

### 2. 循环依赖 init.ts ↔ main.ts（新增）

`main.ts` import `'./init'`，`init.ts` import `'./main'`。esbuild 打包时模块求值顺序不确定，已引发一次 bug（`pdfViewerRegistry` 在 `initContentRenderer` 调用时为 `undefined`）。已通过提取 `pdf-registry.ts` 缓解，但根本结构未变。

**当前状态**：`init.ts` 仍有 1 处 `from './main'`，导入 20 个符号（函数和变量）。只要这个依赖存在，初始化顺序就存在隐患。

**建议**：将 `main.ts` 中剩余的状态变量和工具函数继续迁移到独立模块，最终让 `init.ts` 不再 import `main.ts`。
...
| window.* 全局数量 | 40+ | **0** |
| main.ts 行数 | 3158 | **395** |
| 循环依赖 | 无 | **init↔main（已缓解）** |
| 单测通过率 | — | **724/724 (100%)** |
| 最大单文件 | main.ts 3158 行 | annotation.ts 2183 行 |
| 高风险项 | 2 | **2**（内容已变） |

两个高风险项中，main.ts 已解决，annotation.ts 是新发现的次级问题，循环依赖是重构引入的新风险。
