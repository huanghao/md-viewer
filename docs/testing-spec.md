...
- 与视觉相关的 case，README 可附 `assets/` 下的 gif/png 作为“人工理解材料”，但不作为自动断言基线。

### 2.1 视觉材料与断言分层（低维护成本）

目标：避免“样式小改 -> 大量 case 失效”。

规则：
1. 功能断言优先：断言语义状态与行为（如 `D`/`M`、提示出现、内容是否刷新）。
2. 视觉材料分离：README 中的 gif/png 仅用于说明，不纳入 CI pass/fail。
3. 快照最小化：仅对少量全局关键态使用 screenshot baseline，不在每个 case 大量铺开。
4. 视觉变更流程：如果是预期改版，只更新说明图；只有影响关键视觉契约时才更新快照。

推荐分层：
- L1（默认）：行为断言（低成本，高稳定）
- L2（可选）：README 关键帧/GIF（帮助评审）
- L3（少量）：像素级快照基线（关键页面）
