...
所有"跳转到内容位置"的操作遵循统一规则：

| 场景 | 动画 | 顶部留白 | 实现 |
|------|------|----------|------|
| PDF：TOC 页跳转 | `smooth` | 100px | `scrollToPage` → `container.scrollTo` |
| PDF：翻译条目跳转 | `smooth` | 100px | `scrollToBlockY` → `container.scrollTo` |
| PDF：批注跳转 | `smooth` | 100px | `jumpToAnnotation` → `content.scrollTo` |
| MD：TOC 标题跳转 | `instant` | 90px | `jumpToMdHeading` → `content.scrollTo` |
| Diff 块导航（N/P） | `smooth` | 居中 | `scrollIntoView block:'center'` |
| 页内查找（Cmd+G） | `smooth` | 居中 | `scrollIntoView block:'center'` |

**设计原则：**
- PDF 跳转统一用 `smooth`，留白 100px（目标出现在视口上方约 1/5 处）
- MD TOC 用 `instant`（长文档 smooth 动画过长，体感卡顿）
- Diff / 查找用 `center`（需要看上下文，居中更合适）
- 所有偏移基于 `#content` 的 `scrollTop`，不需要减工具栏高度（`#content` 本身已在工具栏下方）
- `HEADER_OFFSET = 90`（toolbar 48px + tabs ~36px + 余量），见 `main.ts` `jumpToMdHeading`
