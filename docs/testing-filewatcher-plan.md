...
2. 若是当前文件，自动 `syncFileFromDisk`；失败时保留 `M`（由 `lastModified > displayedModified` 体现）。
3. 若是非当前文件，仅更新 `lastModified` 并显示 `M`，等待用户切换时自动同步。
- `file-deleted` 到达后：
...
### P0（回归必跑）
- ✅ `case-10`：`file-changed`（当前自动刷新 + 非当前 `M` + 点击后同步）。
...
- 预期：正文最终稳定为最后一次内容；无长期中间态残留。

### case-17（P1-2）删除后重建恢复
- 前置：A 先打开，删除后进入 `D`。
- 步骤：同路径重建 A，点击“重试加载”（或重新打开）。
- 预期：A 从 `D` 恢复为正常可读；删除提示消失；后续修改继续可观测。
...
2. `M` 状态在自动同步失败时，是否要增加更明确的用户引导文案。
